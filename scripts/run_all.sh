...
#  6_2_2, 8_2_2, 8_4_2, 10_2_2, 10_4_2, 10_6_2, 10_8_2, 10_10_2, 12_2_2,
#  12_4_2, 14_4_2, 16_4_2, 18_4_2, and 20_4_2 under baselines/NLStarRTA/test,
#  first with LearnARTA and then with NLStarRTA.
#
# Synopsis
#  ./scripts/run_all.sh
#
...
if ! find \
  ...
  "$BENCHMARK_ROOT/8_4_2" \
  "$BENCHMARK_ROOT/10_2_2" \
  "$BENCHMARK_ROOT/10_4_2" \
  "$BENCHMARK_ROOT/10_6_2" \
  "$BENCHMARK_ROOT/10_8_2" \
  "$BENCHMARK_ROOT/10_10_2" \
  "$BENCHMARK_ROOT/12_2_2" \
  "$BENCHMARK_ROOT/12_4_2" \
  "$BENCHMARK_ROOT/14_4_2" \
  "$BENCHMARK_ROOT/16_4_2" \
  "$BENCHMARK_ROOT/18_4_2" \
  "$BENCHMARK_ROOT/20_4_2" \
  -mindepth 1 -maxdepth 1 -type f -name '*_*_*-*.json' | IFS= read -r _; then
  printf 'no benchmark files/cases found under benchmark suites in %s. Probably you did not initialize git submodule.\n' "$BENCHMARK_ROOT" >&2
  exit 1
fi
