...
if [ ! -d "${REPO_ROOT}/vendor/symbolicautomata/models/target" ]; then
    echo "symbolicautomata not yet built; running bootstrap-symbolicautomata.sh ..."
    "${REPO_ROOT}/scripts/bootstrap-symbolicautomata.sh" || {
...
# Build drta-size
echo "Building drta-size ..."
MVN="mvn"
$MVN -f "${REPO_ROOT}/tools/drta-size/pom.xml" clean package -q
